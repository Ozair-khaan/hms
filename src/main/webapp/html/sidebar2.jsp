...
if (session.getAttribute("user") != null) {
	...
	if (session.getAttribute("user") != null && isValid2 && session.getAttribute("role_id") != null
	&& session.getAttribute("Entity_id") != null) {
		...
		session.getAttribute("sToken").toString(), session.getAttribute("role_id").toString());
		JSONArray menuandsubMenu = new JSONArray(menuandsub);
%>
<div class="app-admin-wrap layout-sidebar-large clearfix">
	<div class="main-header">
		<div class="logo px-2" style="width: 152px; height: 50px">
			<img src="./assets/images/74235.png" alt="" class="w-100">
		</div>
		...
		<div class="sidebar-left open rtl-ps-none" data-perfect-scrollbar
			...
			<ul class="navigation-left">
				...
				for (int i = 0; i < menuandsubMenu.length(); i++) {
					if (menuandsubMenu.getJSONObject(i).getBoolean("sub_menu")) {
				%>
				<li class="nav-item"
					data-item="menu<%=menuandsubMenu.getJSONObject(i).getInt("menu_id")%>">
					<a class="nav-item-hold" href="#"> <i
						class="<%=menuandsubMenu.getJSONObject(i).getString("menu_icon")%>"></i> <span class="nav-text"><%=menuandsubMenu.getJSONObject(i).getString("menu_name")%></span>
				</a>
					<div class="triangle"></div>
				</li>
				<%
				} else {	
					session.setAttribute("sub_menu_id", menuandsubMenu.getJSONObject(i).getInt("menu_id"));
				%>
				<li class="nav-item"><a class="nav-item-hold"
					href="<%=menuandsubMenu.getJSONObject(i).getString("menu_link")%>">
						<i class="nav-icon i-Bar-Chart"></i> <span class="nav-text"><%=menuandsubMenu.getJSONObject(i).getString("menu_name")%></span>
				</a>
					<div class="triangle"></div></li>
				<%
				}
		...
		<div class="sidebar-left-secondary rtl-ps-none" data-perfect-scrollbar
			...
			for (int i = 0; i < menuandsubMenu.length(); i++) {
				if (menuandsubMenu.getJSONObject(i).getBoolean("sub_menu")) {
					...
					JSONArray innerArray = json.getJSONArray("sub_menu_details");
			%>
			<ul class="childNav"
				...
				for (int j = 0; j < innerArray.length(); j++) {
					;
					session.setAttribute("sub_menu_id", innerArray.getJSONObject(j).getInt("sub_menu_id"));
				%>
				<li class="nav-item"><a
